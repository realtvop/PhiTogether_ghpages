import simphi from "./simphi.js?ver=1.2.2";
import { audio } from "./aup.js?ver=1.2.2";
import {
  full,
  Timer,
  getConstructorName,
  ...
  orientation,
  FrameAnimater,
} from "./common.js?ver=1.2.2";
import { uploader, ZipReader, readFile } from "./reader.js?ver=1.2.2";
import { InteractProxy } from "../../utils/js/interact.js?ver=1.2.2";
import shared from "../../utils/js/shared.js?ver=1.2.2";
        ...
        for (const judgeEvent of list) {
          ...
          if (deltaTime2 > this.time.g) {
            ...
            stat.addDisp(
              Math.max(
                deltaTime2,
                (-1 - note.frameCount) * this.time.AP || 0
              )
            );
...
const hitWordList = new HitEvents({
  ...
  iterateCallback: (i) => {
    ...
    }px ${shared.game.ptmain.gameConfig.useMinaFont ? "Mina" : "Custom, Noto Sans SC"}`;
...
//hit end
const res = {}; //存放资源
let customResourceMeta = { name: "PhiTogether Default 1", author: "Team PhiTogether" };
//初始化
  ...
  async function () {
    ...
    if(ptSettings.resourcesType) {
      if(ptSettings.resourcesType==="pt-custom") {
        try {
          const customResourceMetaXHR = new XMLHttpRequest();
          customResourceMetaXHR.open(
            "GET",
            ptSettings["customResourceLink"],
            false
          );
          customResourceMetaXHR.send();
          customResourceMeta = JSON.parse(customResourceMetaXHR.responseText);
    ...
    const erc = (str) => {
      if (ptSettings.resourcesType==="pt-custom") {
        ...
        if (customResourceMeta["includesHitSongs"] && hitSongs.indexOf(str) > -1) return customResourceMeta["res"][str];
      }
      return pth + pack + "/" + str;
    };
    await Promise.all(
    ...
    for (const i of entries)
      await noteRender.update(i, res[i], 1);
    await noteRender.updateFX(
      res["HitFXRaw"],
      1
    );
    ...
    if (ptSettings.resourcesType === 'prpr-custom') {
      try {
        ...
        shared.game.msgHandler.sendMessage('错误：无法应用自定义资源包，界面显示可能异常，请检查设置', 'error')
      }
    }
...
function mainLoop() {
  ...
  if (qwqOut.second < 0.67) {
    ...
    setTimeout(() => {
      ...
      audio.play(
        res[
          `LevelOver${ shared.game.ptmain.gameConfig.usePClevelOverbgm ? (difficulty < 0 ? 2 : difficulty) : "0"}${
            shared.game.ptmain.gameConfig.usePClevelOverbgm ? "_v2" : ""
          }`
        ],
        { loop: true }
      );
  ...
  //Copyright
  ctx.font = `${lineScale * 0.4}px ${
    shared.game.ptmain.gameConfig.useMinaFont ? "Mina" : "Custom, Noto Sans SC"
  }`;
  ctx.fillStyle = "#FFF";
  ...
  ctx.fillText(
    `${
      app.playMode===1?"[Autoplaying] ":""
    }PhiTogether v${spec.thisVersion} by Team PT on sim-phi - P${
      judgeManager.time.p * 1000
    }G${judgeManager.time.g * 1000}S${app.speed.toFixed(2)} - ${
      shared.game.ptmain.noAccountMode ? "OFF" : "ON"
...
function loopCanvas() {
  ...
  ctxos.resetTransform();
    ...
    const waterMark = "PhiTogether Insider Version";
		ctxos.font = `${lineScale * 2}px ${
      shared.game.ptmain.gameConfig.useMinaFont ? "Mina" : "Custom, Noto Sans SC"
    }`;
		let textWidth = ctxos.measureText(waterMark).width;
		if (textWidth > (canvasos.width / 1.5)) {
			ctxos.font = `${lineScale * 2 * ((canvasos.width / 1.5) / textWidth)}px ${
        shared.game.ptmain.gameConfig.useMinaFont ? "Mina" : "Custom, Noto Sans SC"
      }`;
		} else {
			ctxos.font = `${lineScale * 2}px ${
  ...
  if (qwqIn.second < 3) {
    ...
    if (dxt > canvasos.width - lineScale * 1.5)
      ctxos.font = `${
        ((lineScale * 0.55) / dxt) * (canvasos.width - lineScale * 1.5)
      }px ${
        shared.game.ptmain.gameConfig.useMinaFont ? "Mina" : "Custom, Noto Sans SC"
      }`;
  ...
  ctxos.textBaseline = "alphabetic";
  ctxos.textAlign = "right";
  ctxos.font = `${lineScale * 0.63}px ${
    shared.game.ptmain.gameConfig.useMinaFont ? "Mina" : "Custom, Noto Sans SC"
  }`;
  const dxlvl = ctxos.measureText(levelText).width;
  if (dxlvl > app.wlen - lineScale)
    ctxos.font = `${((lineScale * 0.63) / dxlvl) * (app.wlen - lineScale)}px ${
      shared.game.ptmain.gameConfig.useMinaFont ? "Mina" : "Custom, Noto Sans SC"
    }`;
  ctxos.fillText(
    tmps.level,
  ...
  if (dxsnm > app.wlen - lineScale)
    ctxos.font = `${((lineScale * 0.63) / dxsnm) * (app.wlen - lineScale)}px ${
      shared.game.ptmain.gameConfig.useMinaFont ? "Mina" : "Custom, Noto Sans SC"
    }`;
  ctxos.fillText(
    inputName.value || inputName.placeholder,
...
emitter.addEventListener(
  ...
  /** @this {Emitter} */ function () {
    ...
    if (this.eq("play")) {
      ...
      app.playMode = shared.game.ptmain.gameConfig.idn1 && shared.game.ptmain.gameConfig.account && shared.game.ptmain.gameConfig.account.userBasicInfo.isPTDeveloper ? 1 : 0;
    }
    // console.log(this);
  }
);
