...
import { renderPZApiFromCache } from '../cachemanage/cacheutils.js?ver=1.2.2';
export default {
    name: 'chartSelect',
    template: `
    <div>
      <div style="display:flex;justify-content: center;flex-direction:column;align-items:center;">
        ...
        <div id="chartListAll">
          ...
          >
            <img :src="imgCompLink(chart.illustration.replace('res.phi.zone',pzResUrlGlobal))" />
            <div class="songCardCover" :style="{lineHeight: '1.2em', '--bg':'url('+imgCompLink(chart.illustration.replace('res.phi.zone',pzResUrlGlobal))+')'}">
  ...
  `,
    ...
    computed: {
        canPrev() {
          if(['pz', 'favorite'].includes(this.selectChoice)) return this.chartList.previous?true:false;
          else if(this.selectChoice === 'local') return (this.page-1>0);
        },
        canNext() {
          if(['pz', 'favorite'].includes(this.selectChoice)) return this.chartList.next?true:false;
    ...
    methods: {
        ...
        loadPrevPage() {
          if(['pz', 'favorite'].includes(this.selectChoice)) this.loadPage(this.chartList.previous);
          else if(this.selectChoice === 'local') {
        ...
        loadNextPage() {
          if(['pz', 'favorite'].includes(this.selectChoice)) this.loadPage(this.chartList.next);
          else if(this.selectChoice === 'local') {
            this.page = this.page+1;
            this.updatePagination();
          }
        ...
        async loadOffline() {
          if(this.forceOffline)this.selectChoice = 'local';
        ...
        doSearch() {
            if(this.selectChoice === 'pz') {
              this.loadPage(
                shared.game.ptmain.gameConfig.apiSource+`/songs/?query_charts=1&query_levels=1${
              ...
              this.loadPage(
                  shared.game.ptmain.gameConfig.apiSource+`/songs/?query_charts=1&query_levels=1&id=${this.generateFavoriteList()}${
                      this.search.name ? '&name=' + this.search.name : ''
                  }${this.search.composer ? '&composer=' + this.search.composer : ''}${
                      this.search.illustrtor ? '&illustrtor=' + this.search.illustrtor : ''
                  }`,
        ...
        async loadPage(url, renew = false) {
            ...
            try {
                ...
                const chartList = await (
                    await fetch(url.replace(/https?:\/\/api.phi.zone/, shared.game.ptmain.gameConfig.apiSource), { headers: myHeaders })
                ).json();
        ...
        imgCompLink(a) {
            if (this.selectChoice !== 'pz' || a.includes('PTVirtual')) return a;
